# 내장 템플릿 16종 실측표 (v1.10.1 기준 — 종수는 v0.69 이후 그대로)
...
**열린 막까지 누적**으로 메인에게만 주입된다 (밝혀진 진실은 잊히지 않는다 — `scenario.js` `acts.slice(0, idx+1)`).

### rpg — secrets '아린의 과거' 3단계 (v1.10.0, 유일한 비밀 실물)
```
id arin   kind person   about 아린   label 아린의 과거   (tell 기본 = exists)
0단계 낌새  when 없음(시작부터)  "아린은 감시탑 문장을 보면 시선을 피한다. 봉인 이야기가 나오면 화제를 돌린다."
1단계 부분  when scn_act == "shadow"  "아린은 감시탑 출신이다. 탑을 떠난 이유는 아직 말하지 않는다."
                                    notify "[비밀] 아린이 자신이 감시탑 출신임을 털어놓았다."
2단계 전모  when scn_act == "clash"   "봉인을 삭게 만든 배신의 한복판에 아린의 스승이 있었다. …"
                                    notify "[비밀] 아린이 스승의 배신과 자신의 죄책감을 고백했다."
```
**막과 맞무는 정석 = `when: scn_act == "<막 id>"`** — 시나리오의 막 secret(세계의 진상)은 시나리오 탭에 그대로 두고,
인물의 비밀만 여기로. 0단계는 **이유 없는 행동만** 적는다(왜 피하는지는 2단계까지 없다). 1막을 도는 동안 1·2단계 문장은
메인에게 한 글자도 안 간다 — `test-secret.js`가 grep으로 증명. 배포글 예시는 이 값을 그대로 쓴다.
