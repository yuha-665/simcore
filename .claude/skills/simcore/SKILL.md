---
name: simcore
description: RisuAI/PocketRisu용 시뮬레이션 엔진 플러그인 SimCore의 개발·유지보수 레퍼런스 (v1.12.0 기준). 스키마 전 필드(전투 안무·상점·보드·메신저·의뢰판·시나리오·🔒 비밀·⏪ 되감기·🎭 무대 뒤 포함), 편집기 18탭 UI 구조, 내장 템플릿 16종 실측표, 리수 내부 확정 사실, 아카라이브 배포글 규격, 얼헌(이식 봇) 개조 번들 규약을 담는다. simcore.plugin.js를 고치거나, 베리디아·얼헌 같은 SimCore 봇 스키마를 만들거나, SimCore 가이드/패치글을 쓸 때 사용.
---

# SimCore — 시뮬 엔진 플러그인

봇에 **숫자로 굴러가는 상태**를 붙이는 RisuAI 플러그인. 스키마 하나로 변수·파생·규칙·이벤트·상태창을
정의하면, 메인 모델에는 상태 블록을, 보조 모델에는 갱신 지시를 내보낸다. (이 문서 **v1.12.0 기준, 2026-09-25**)
...
**v1.12 🎭 무대 뒤 — 안 봐도 세상은 움직인다**(`core/front.js`, 진영 시계·징후·밑작업·표면화). 얼헌(얼터헌터) 개조가
...
| 설계 문서 | `simcore\docs\design-*.md` (시간·시나리오레이터·에셋 슬롯·편성표·모듈 애드온·UI 크롬·**비밀**(`design-비밀.md`, §9 미결: SECTIONS 패치 합류·새 시작 페이지 단계 강제 열기/닫기)…) + `docs\ai-mistakes.md` |
...
**정본 브랜치는 `master`**, 기능은 `feat/*`에서 만들고 **집 실기(리수에서 돌려 봄) 뒤 `--no-ff`로 머지**한다 (v1.10.0 비밀 = `feat/secret`, 2026-09-17 실기 대기). **작업 전 pull 먼저** — 집·회사가 번갈아 민다.
...
- `테스트/test-aischema.js`의 **규격서 크기 상한**은 검증기 소스를 그대로 싣는 구조라 **검증기가 자라면
  같이 자란다** — 여덟 번 올린 끝에 "붙여넣기 상식선" **128KB** 하나로 못 박았다 (현재 96.5KB). 넘으면 상한을 올리지 말고 규격서에서 뺄 걸 찾는다.
...
| 문서 | 내용 |
|---|---|
| [references/schema.md](references/schema.md) | 스키마 전 필드 · 타입 · 검증 규칙 · 표현식 문법 (전투 안무 `checks[].fight` v1.6 · 상점 units/exchange/shops v1.3~1.4 · 막간 v1.5 · 의뢰판 v1.7.9 · liveChoices v1.8 · **secrets v1.10** 포함) |
| [references/editor.md](references/editor.md) | 편집기 **18개 탭**(v1.12.0 [무대 뒤] 포함 — editor.md 본문은 17탭 기준)의 UI 라벨과 가져오기 동작 (`TAB_SLICES`), 3층 구조, 기능 추가 카드, v1.9.28 번호 섹션(`sce-board-*`) 규약 |
...
1. **`beforeRequest` 리플레이서에서 `type`을 안 보는 것.** 앱의 **모든** LLM 요청에 걸리므로
   ...
   `afterRequest`(v1.10.2부터 씀)도 똑같다 — 모든 성공 응답에 걸리고 호출부에 try/catch가 없다. 우리는 `translate`만 만진다.
   거꾸로, **전 타입에서 떼는 마커는 번역문에서도 사라진다** — LLM 번역 + [HTML 포맷 전 번역]이면 상태창이 번역 뒤에 그려지므로
   뗀 번호를 afterRequest에서 도로 붙여야 한다 (v1.10.2 실사고, 짝짓기 규칙은 risu-facts.md "번역은 저장본을 안 건드린다").
2. **보조 호출을 system 한 통으로 보내는 것.** 구글 계열에서 죽는다. 짧은 user 턴을 붙일 것. (v0.37.1 사고)
   ...
   **변수 수도 상한을 민다** (v1.9.8 실사고 — 롤 프로게이머 시뮬, 변수 51·목록 18): 10명짜리 목록 일곱이 remove+add로
   다시 써지면 JSON만 3천 토큰인데 400 고정에 잘려 통째로 버려졌고 "변수가 하나도 안 변하고 제안도 안 뜬다"가 됐다.
   지금은 `engine.auxOutputBudget`(열린 변수만큼 예산, 천장 10000 · 보조 추론 2048)과 `salvageTruncatedJson`(잘려도 완성된 항목까지 살림)이
   막는다. "변수를 줄이니 다시 된다"는 제보가 오면 변수 탓이 아니라 **출력이 잘린 것**부터 의심 — 패널 [보조 모델] 상태줄의 ⚠.
...
13. **에셋 `usage`에 "감정이 크게 변할 때만" 같은 게이트 문구.** 모델은 히스토리를 모방한다 — 앞선 응답에 이미지가 0장이면
    ...
    같은 콜드 스타트가 **엔진 쪽에서도** 났다 — 설정 턴(turn 0)엔 에셋 주입문을 안 붙이던 옛 가정("세션 0은 설정 대화") 때문에
    설정 턴이 곧 첫 장면인 봇(아틀리에)은 첫 응답이 0장이었다 (v1.7.11에서 설정 턴에도 붙임). 첫 응답에 이미지가 없으면 지침을 의심하기 전에 **turn 0 promptBlock에 `[Image tags]`가 있는지**부터 본다.
14. **루트 문서(리수 메인 DOM)에 z-index 50 이상 요소를 넣는 것.** 리수 권한 팝업이 z-index 50이라 그 위에 얹히면 유저가 권한 동의를 못 한다 (커뮤니티 제보, 2026-09-06).
    SimCore의 큰 z-index(#sc-root·#sc-game 2147483000)는 전부 **플러그인 iframe 안 document**라 메인 DOM과 무관하고, 메시지 안 상태창은 최대 5(팝오버)다 — 이 상태를 유지할 것.
    루트 문서엔 요소를 넣지 않는다(getRootDocument는 클릭 히트테스트 읽기 전용). 단 우리 전체화면 컨테이너(리수 것)가 떠 있는 동안은 리수 alert·권한창이 가려지므로 규칙 #6대로 패널이 닫힌 뒤에 부른다.
15. **배포 번들이 남의 카드 정규식을 통째로 덮는 것.** 번들 `regex`는 통째 교체 계약(얼헌식 — 원본 정규식 54종을 심코어판으로 갈아 끼우는 용도)이라,
    "원본 카드엔 정규식이 없으니 괜찮다"고 실었다가 유저 카드의 **에셋 표시 정규식까지 날려 이미지가 안 나왔다** (아틀리에 실사고, 2026-09-06).
    원본에 없다는 건 배포받는 쪽 카드에 대한 가정일 뿐이다. 몇 개 얹을 땐 **`regexAdd`**(v1.7.10 — 카드 정규식 유지, 같은 comment만 교체, 뒤에 덧붙임)로.
    날아갔으면 💾 [교체 되돌리기](적용 전 백업 최근 3개, 한 번에 한 단계)로 복구.
16. **잘라 내는 상자(`overflow:hidden`) 안에 조작 버튼을 두는 것.** 칩 `white-space:nowrap`(칸 밖으로 자란다) + 감싼 상자 `overflow-x:hidden`(칸 밖을 자른다)이
    겹치면 칩 맨 끝의 ✕부터 사라진다 — 에러도 경고도 없이 "지울 수 없다"가 된다 (v1.10.1 실사고, 현황 탭 목록 변수). 자르기를 떼는 게 답이 아니라
    (가로 스크롤이 생겨 ✕가 화면 밖으로 간다) **넘칠 일을 없앤다** — 글자만 접고 버튼·숫자는 `flex:0 0 auto`. `#sc-vars table`은 `table-layout:fixed`라
    행마다 열 폭을 못 바꾼다(넓혀야 하면 `colSpan`뿐). 패널 스타일시트엔 같은 선택자가 **세 번**(옛 팔레트·v1.9 토큰·`@media`) 나오니 테스트는 캐스케이드로 본다 (`test-varchip.js`). 상세: risu-facts.md.
...
- **`skip_day`는 시각을 보존한다 — 잠드는 액션에 `skip_day + 1`을 쓰지 마라.** 18:00에 자면 이튿날 18:00이 되고, 보조가 아침에
  맞추려 분을 얹다 23:15 같은 시각이 난다 (아틀리에 2026-09-07 실기). 잠은 `skip_min = ((기상시*60 - (hour*60+minute)) + 1439) % 1440 + 1`
  ("다음으로 돌아오는 그 시각", 새벽엔 같은 날) — `time.expose`에 hour·minute가 있어야 한다. 시간대를 서사가 고르는 봇은
  로맨스의 `wake_at`+`day_break_sync` 꼴, 아침 고정 봇은 식 하나면 된다.
...
- **비밀은 "말하지 마라"가 아니라 프롬프트에서 뺀다** (v1.10.0 `secrets`). 프롬프트에 적힌 건 전부 "아는 것"이고 금지문은 그 옆의 텍스트 하나라
  100턴이면 한 번은 샌다. 단계(낌새 → 부분 → 전모)의 `when`이 열릴 때까지 그 `text`는 어디에도 안 실린다. **카드·페르소나·로어북에 적힌 비밀은
  이미 새고 있다** — 그쪽에서 잘라 내지 않으면 켜도 샌다(기능의 절반이 이 안내). 낌새는 **이유 없는 행동만**("문장을 보면 움찔한다", 왜는 안 준다),
  존재는 알리되 내용은 안 준다(`tell:'exists'`, 없으면 모델이 빈칸을 지어낸다), 반전(plot)만 존재 신호가 곧 스포일러라 기본 `tell:'none'`.
  보조 모델엔 아무것도 안 간다 — 변수만 세우면 조건이 알아서 찬다. 막의 `secret`(시나리오)은 이것의 막 묶음 특수형이고 `scn_act == "act3"`로 맞물린다.
