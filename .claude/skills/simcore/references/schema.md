...
| `exchange` (v0.97) | `{ var, rate, spread?, label? }` — 통화 ↔ 다른 숫자 지갑 환전 창구. 통화 1 = rate(상대 지갑), 살 때 ×(1+spread)·팔 때 ×(1-spread) (spread 기본 0.2). 계산·정산 전부 엔진, 보조 호출 0. 통지 접두는 `label` (상점 본체와 다른 장소일 수 있다 — 얼헌 "암거래 환전") |
