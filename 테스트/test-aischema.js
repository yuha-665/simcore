...
// ── 규격서 ──
{
  ...
  //
  // ⚠ 이 상한은 **성장 감지기가 아니다** (v1.7.0에서 성격 정정). 규격서는 검증기 원문을 그대로
  // 싣는 구조라 기능이 늘 때마다 반드시 자란다 — 그래서 60→72→76→80→88→92→94→95→96KB로
  // 여섯 번을 "기능 추가 = 상한 조정"으로 따라다녔고, 그 사이 이 단언은 회귀를 한 번도 못 잡았다.
  // 매번 올릴 수밖에 없는 눈금은 규율이 아니라 잡일이다.
  //
  // 그래서 상한을 **붙여넣기 상식선** 하나로 못 박는다: 128KB (현재 96.5KB — 여유 31KB).
  // 여기 닿으면 그건 또 올릴 때가 아니라 규격서를 절로 쪼갤 때다 (검증기 원문 동봉을 재고할 것).
  //
  // ⚠ 아래 비례 단언은 **감시가 아니다**. 번들이 1.5MB까지 자라 상한이 380KB로 헐거워졌다 —
  // 규격서(96KB)와 284KB나 벌어져 있어 사실상 항상 참이다. 옛 주석의 "전체 소스 251KB"도
  // 그 시절 숫자였다. 남겨는 두되(규격서만 폭주하는 병리는 여전히 잡는다) 이걸 성장 감시로
  // 믿지 말 것 — 실질 눈금은 위의 128KB 하나뿐이다.
  const full = buildSchemaSpecPrompt('politics', true);
  ck('★ 규격서가 붙여넣기 가능한 크기 (128KB 미만 — 닿으면 상한을 올리지 말고 규격서를 쪼갤 것)',
    full.length < 128 * 1024, kb(full));
  ck('번들 대비 비례 (헐거운 하한선 — 규격서만 폭주하는 경우만 잡힌다)',
    full.length < src.length / 4, `${kb(full)} vs ${kb(src)}`);
