...
// ── 검증 ──
{
  ...
  // v0.79부터 문장이 둘이다 — 한 명이면 '먼저 선언된 팩이 우선', 여럿이면 팩 단위로 접은
  // '먼저 선언된 쪽이 담당하므로 … 안 쓰입니다'. 있고 없고를 재는 자리라 둘 다 잡는다.
  const isShadow = (w) => /먼저 선언된 (팩이 우선|쪽이 담당)/.test(w.msg);
  const overlap = snap(); overlap.assets.packs[2].chars = ['Hiromi'];
  const vo = validateSchema(overlap);
  ck('★ 인물 중복 담당은 경고 + 먼저 선언 우선 안내', vo.ok && vo.warnings.some(isShadow), '');
  ...
  ck('★ 게이트 다른 변형 팩 공존은 경고 없음 (성인/임신 변형 패턴)',
    !validateSchema(snap()).warnings.some(isShadow), '');
  const sameGate = snap(); sameGate.assets.packs[1].when = undefined; // nsfw 게이트 제거 → 진짜 충돌
  ck('게이트까지 같으면 여전히 경고', validateSchema(sameGate).warnings.some(isShadow), '');
