  ...
  directives: [
    { id: 'pov_rosetta', when: 'pov == "rosetta"',
      text: '[시점] 유저는 로제타 비올라 카르디온의 몸에 빙의한 현대인이다. 모든 서술·대사에서 유저를 로제타로 부르고, 주변 인물은 유저를 로제타로 대한다. '
        + '유저의 원래 이름이나 "플레이어"·"유저" 같은 말은 본문에 쓰지 마라. 페르소나에 빙의자(안에 든 현대인)의 성별·직업·성격이 적혀 있으면 그건 속마음·말버릇·아는 것에만 드러내고, 세상은 로제타로 대한다. 빙의자는 원작 소설을 읽었다.' },
    { id: 'pov_servant', when: 'pov == "servant"',
      text: '[시점] 유저는 원작에 이름 한 줄 없는 인물의 몸에 빙의한 현대인이다 — 몸의 이름·성별·출신은 페르소나를 따른다. '
        + '페르소나에 빙의자(안에 든 현대인)의 성별·직업·성격이 적혀 있으면 그건 속마음·말버릇·아는 것에만 드러내고, 세상은 몸의 주인으로 대한다. 빙의자는 원작 소설을 읽었다. '
...
console.log('\n━━ 시종 시점 — 첫 출근 날 ━━');
{
  ...
  ok('빙의자 칸: 속마음에만 · 세상은 몸의 주인으로 (로제타 시점은 로제타로)', p0.includes('속마음·말버릇·아는 것에만 드러내고, 세상은 몸의 주인으로 대한다')
    && send(start('rosetta')).promptBlock.includes('세상은 로제타로 대한다'), '');
