...
console.log('── 패치');
{
  ...
  ck('update가 when을 빠뜨리면 기존 when 유지 (v1.9.17 병합)', r2.ok && r2.schema.directives[0].when === 'iron > 5' && r2.schema.directives[0].text === '철 이야기.', J(r2.errors));
  ck('그땐 경고 없음', r2.ok && !r2.applied.warnings.some((w) => w.includes("update.directives 'd_old'")), '');
  const r2n = apply({ update: { directives: [{ id: 'd_old', when: null }] } });
  ck('when: null 로 지우면 true로 채우고 경고', r2n.ok && r2n.schema.directives[0].when === 'true' && r2n.applied.warnings.some((w) => w.includes("update.directives 'd_old'")), J(r2n.errors));
